            ...
            switch (preset)
            {
                ...
                case Preset.PLD_RetargetSheltron_TT:
                    ...
                    ImGuiEx.TextWrapped(ImGuiColors.DalamudGrey,
                        ...
                        "如果不这样做，干预将会替换连击，并施放到主防护职业身上。\n" +
                        "如果你不打算用这些功能来进行个人减伤，建议不要启用此选项。");
                    ImGui.Unindent();
                    break;
                case Preset.PLD_RetargetShieldBash:
                    DrawAdditionalBoolChoice(PLD_RetargetStunLockout, "锁定技能", "如果没有可被眩晕的目标，则用狂怒剑锁定该技能");
                    if (PLD_RetargetStunLockout)
                        DrawSliderInt(1, 3, PLD_RetargetShieldBash_Strength, "在眩晕已被施加指定次数后锁定");
                    break;
